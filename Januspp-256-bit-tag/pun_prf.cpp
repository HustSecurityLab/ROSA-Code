...
{
    this->type = NORMAL;
    array<unsigned char, 16> data_ = {};

    memcpy(data_.data(), data, 16);
    this->keydata.emplace_back(data_);
}
...
size_t PuncturedKey::size() const
{
    size_t ret = 0;

    ret = 16 * this->keydata.size();
    ret += 2 * ((4 * sizeof(unsigned long long)) * this->tag_prefix.size());
...
int PuncturedKey::hash(unsigned char *hash_out)
{
    ...
    SHA256(this->keydata[0].data(), 16, buf1);
    for (size_t i = 1; i < this->keydata.size(); i++)
    {
        memcpy(buf1 + 32, this->keydata[i].data(), 16);
        SHA256(buf1, 48, buf2);
        memcpy(buf1, buf2, 32);
    }
...
void PuncturedKey::dump_data(FILE *f_out)
{
    ...
    for (auto &itr: this->keydata)
        fwrite(itr.data(), sizeof(char), 16, f_out);
...
int PuncturablePRF::Eval(PuncturedKey &key, PunTag &tag, unsigned char *out_result)
{
    if (key.type == NORMAL)
    {
        this->Eval(key.keydata[0].data(), tag, out_result);
        return 1;
    }
    else
    {
        for (int i = 0; i < (int) key.tag_prefix.size(); i++)
        {
            ...
            {
                this->_Eval_from_path(key.keydata[i].data(), tag, i + 1, out_result);
                return 1;
            }
        }
        return 0;
...
int PuncturablePRF::Punc(unsigned char *sk, PunTag &tag, PuncturedKey &out_pun_key)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        out_pun_key.keydata.emplace_back(array<unsigned char, 16>());
        ...
        if (traveled_mask.is_0())
        {
            ...
            aes_cbc_prf(out_pun_key.keydata[0].data(), sk, data_to_enc);
        ...
        else
        {
            ...
            if (i % 2 == 0)
                aes_cbc_prf(out_pun_key.keydata[i].data(), tmp1, data_to_enc);
            else
                aes_cbc_prf(out_pun_key.keydata[i].data(), tmp2, data_to_enc);
