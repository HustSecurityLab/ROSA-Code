...
void Benchmark::benchmark_test_DataUpdate()
{
    ...
    int num_data_to_del = MAX_DELETESUPPORT;
    int updated_num = 0;
...
void Benchmark::benchmark_test_Search()
{
    ...
    for (auto &itr : data_to_encrypt)
    {
        ...
        cout << "Searching with Januspp-256 for keyword: " << itr.first << endl;
        cout << "\tTotally find " << result_size << " records" << endl;
    ...
    for (int por = 0; por < 101; por += 10)
    {
        ...
        if (por != 0)
        {
            random_select_file_identifiers_(id_to_del, id_index, keyword_to_delete,
                                            int(por / 100.0 * MAX_DELETESUPPORT));
        ...
        cout << "Searching with Deletion with Januspp-256 for keyword: " << keyword_to_delete << endl;
...
{
    int keyword_num = 0;
    int clnt_stor, srv_stor, srv_del_stor, oldres_stor;
    auto channel = grpc::CreateCustomChannel(this->addr, grpc::InsecureChannelCredentials(),
                                             get_channel_args());
    SSEClient sse_client(channel, this->addr);
    ...
    while (keyword_num <= 5000)
    {
        ...
        {
            sse_client.GetStor(clnt_stor, srv_stor, srv_del_stor, oldres_stor);
...
void Benchmark::get_srv_stor(int entry_num)
{
    ...
    sse_client.GetStor(clnt_stor, srv_stor, srv_del_stor, oldres_stor);
    cout << "Entry number: " << entry_num << ", srv_stor: " << srv_stor << " Bytes, srv_del_stor: " << srv_del_stor << " Bytes, oldres_stor: " << oldres_stor << " Bytes, Total: " << srv_stor + srv_del_stor + oldres_stor << " Bytes." << endl;
    sse_client.Search("keyword");
    sse_client.GetStor(clnt_stor, srv_stor, srv_del_stor, oldres_stor);
    cout << "After search, srv_stor: " << srv_stor << " Bytes, srv_del_stor: " << srv_del_stor << " Bytes, oldres_stor: " << oldres_stor << " Bytes, Total: " << srv_stor + srv_del_stor + oldres_stor << " Bytes." << endl
         << endl;
}

void Benchmark::benchmark_stor()
{
    cout << "----------------[Client Storage of JanusPP-256]------------------" << endl;
    get_client_stor();
    cout << "----------------[Server Storage of JanusPP-256]------------------" << endl;
