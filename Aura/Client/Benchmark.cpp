...
void Benchmark::benchmark_test_DataUpdate()
{
    ...
    cout << "\taverage time " << total_del / num_data_to_del << " us" << endl
         << endl;
    // cout << "\taverage client time " << total_del / num_data_to_del << " us" << endl
    //     << endl;
}
...
void Benchmark::benchmark_test_Search()
{
    ...
    for (auto &itr : data_to_encrypt)
    {
        ...
        cout << "Searching with Aura for keyword: " << itr.first << endl;
...
void Benchmark::benchmark_test_delete(const std::string &keyword_to_delete)
{
    ...
    for (int por = 0; por < 91; por += 10)
    {
        ...
        cout << "Searching with Deletion with Aura for keyword: " << keyword_to_delete << endl;
...
{
    int keyword_num = 0;
    int clnt_stor, EDB_stor, Cache_stor;
    auto channel = grpc::CreateCustomChannel(this->addr, grpc::InsecureChannelCredentials(),
                                             get_channel_args());
    SSEClient sse_client(channel, this->addr);
    ...
    while (keyword_num <= 5000)
    {
        ...
        {
            sse_client.GetStor(clnt_stor, EDB_stor, Cache_stor);
...
void Benchmark::get_srv_stor(int entry_num)
{
    ...
    sse_client.GetStor(clnt_stor, EDB_stor, Cache_stor);
    cout << "Entry number: " << entry_num << ", EDB size: " << EDB_stor << " Bytes, Cache size: " << Cache_stor << " Bytes, Total: " << EDB_stor + Cache_stor << " Bytes." << endl;
    sse_client.Search("keyword");
    sse_client.GetStor(clnt_stor, EDB_stor, Cache_stor);
    cout << "After search, EDB size: " << EDB_stor << " Bytes, Cache size: " << Cache_stor << " Bytes, Total: " << EDB_stor + Cache_stor << " Bytes." << endl
         << endl;
}

void Benchmark::benchmark_stor()
{
    cout << "----------------[Client Storage of Aura]------------------" << endl;
    get_client_stor();
    cout << "----------------[Server Storage of Aura]------------------" << endl;
