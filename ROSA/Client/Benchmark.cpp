...
void Benchmark::benchmark_test_DataUpdate()
{
    ...
    SSEClient sse_client(channel, OMAP_CAP, this->addr);
    chrono::steady_clock::time_point begin, end;
    chrono::duration<double, std::micro> elapsed;
    double total_add = 0, total_del = 0, clnt_add = 0, clnt_del = 0;
    ...
    for (auto &itr : data_to_encrypt)
    {
        ...
        {
            sse_client.Update(itr.first, id, RosaAdd);
            updated_num++;
    ...
    {
        sse_client.Update(itr.first, itr.second, RosaDel);
    }
    end = chrono::steady_clock::now();
    elapsed = end - begin;
    ...
    cout << "\taverage time " << total_add / total_entry_num << " us" << endl
         << endl;
    ...
    cout << "\taverage time " << total_del / num_data_to_del << " us" << endl
         << endl;
...
void Benchmark::benchmark_test_Search()
{
    ...
    for (auto &itr : data_to_encrypt)
    {
        ...
        begin = chrono::steady_clock::now();
        int result_size = sse_client.Search(itr.first);
        end = chrono::steady_clock::now();
        elapsed = end - begin;
        total_time = elapsed.count();

        cout << "Searching with ROSA for keyword: " << itr.first << endl;
        cout << "\tTotally find " << result_size << endl;
        ...
        cout << "\tAverage time cost is " << fixed << total_time / result_size << " us" << endl;
    }
}
...
void Benchmark::benchmark_test_delete(const std::string &keyword_to_delete)
{
    chrono::steady_clock::time_point begin, end;
    chrono::duration<double, std::micro> elapsed;
    vector<string> id_to_del, Ls, Tags;
    ...
    for (int por = 0; por < 91; por += 10)
    {
        ...
        if (por != 0)
        {
            ...
            for (auto &itr : id_to_del)
                sse_client.Update(keyword_to_delete, itr, RosaDel);
        }
        ...
        begin = chrono::steady_clock::now();
        int result_size = sse_client.Search(keyword_to_delete);
        end = chrono::steady_clock::now();
        elapsed = end - begin;
        total_time = elapsed.count();

        cout << "Searching with Deletion with ROSA for keyword: " << keyword_to_delete << endl;
        cout << "Delete portion: " << por * 0.01 << ", deleted "
             << id_to_del.size() << endl;
        cout << "\tTotally find " << result_size << endl;
        cout << "\tTime cost of client is " << std::fixed << bench_clnt_time << " us, average is "
             << bench_clnt_time / result_size << " us" << endl;
        cout << "\tTime cost of the whole search phase is " << fixed << total_time << " us" << endl;
        cout << "\tAverage time cost is " << fixed << total_time / result_size << " us" << endl;
    }
}
...
{
    int keyword_num = 0;
    int clnt_stor, CDB_stor, Cache_stor, OMAP_stor;
    auto channel = grpc::CreateCustomChannel(this->addr, grpc::InsecureChannelCredentials(),
                                             get_channel_args());
    SSEClient sse_client(channel, OMAP_CAP, this->addr);
    ...
    {
        sse_client.Update("keyword" + to_string(keyword_num), "file-" + to_string(keyword_num), RosaAdd);
        keyword_num += 1;

        if (keyword_num % 250 == 0)
        {
            sse_client.GetStor(clnt_stor, CDB_stor, Cache_stor, OMAP_stor);
...
void Benchmark::get_srv_stor(int entry_num)
{
    ...
    auto channel = grpc::CreateCustomChannel(this->addr, grpc::InsecureChannelCredentials(),
                                             get_channel_args());
    SSEClient sse_client(channel, entry_num, this->addr);

    sse_client.Setup();

    for (int i = 0; i < entry_num; i++)
    {
        sse_client.Update("keyword", "file-" + to_string(i), RosaAdd);
    }
    sse_client.GetStor(clnt_stor, CDB_stor, Cache_stor, OMAP_stor);
    cout << "Entry number: " << entry_num << ", CDB size: " << CDB_stor << " Bytes, Cache size: "
         << Cache_stor << " Bytes, OMAP size: " << OMAP_stor << " Bytes, Total: "
         << CDB_stor + Cache_stor + OMAP_stor << " Bytes." << endl;
    sse_client.Search("keyword");
    sse_client.GetStor(clnt_stor, CDB_stor, Cache_stor, OMAP_stor);
    cout << "After search, CDB size: " << CDB_stor << " Bytes, Cache size: " << Cache_stor
         << " Bytes, OMAP size: " << OMAP_stor << " Bytes, Total: "
         << CDB_stor + Cache_stor + OMAP_stor << " Bytes." << endl
         << endl;
}
...
{
    cout << "----------------[Client Storage of ROSA]------------------" << endl;
    get_client_stor();
    cout << "----------------[Server Storage of ROSA]------------------" << endl;
